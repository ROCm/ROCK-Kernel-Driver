...
static struct dentry *lookup_xattr(struct inode *inode, const char *name,
                                   int flags)
{
...
static struct file *open_xattr_file(struct inode *inode,
                                    const char *name, int flags)
{
	struct dentry *dentry = lookup_xattr(inode, name, flags);
			  ...
			  const void *buffer, size_t buffer_size, int flags)
{
	...
	if (!buffer) {
		...
		dentry = lookup_xattr(inode, name, XATTR_REPLACE);
		if (IS_ERR(dentry)) {
			err = PTR_ERR(dentry);
			if (err == -ENODATA)
				err = 0;
