/*
  ...
  $Id: core_async.c,v 1.4 2004/02/25 00:35:15 roland Exp $
*/
...
typedef struct tTS_IB_ASYNC_EVENT_HANDLER_STRUCT tTS_IB_ASYNC_EVENT_HANDLER_STRUCT,
  *tTS_IB_ASYNC_EVENT_HANDLER;

typedef struct tTS_IB_ASYNC_EVENT_LIST_STRUCT tTS_IB_ASYNC_EVENT_LIST_STRUCT,
  *tTS_IB_ASYNC_EVENT_LIST;

struct tTS_IB_ASYNC_EVENT_HANDLER_STRUCT {
  TS_IB_DECLARE_MAGIC
...
struct tTS_IB_ASYNC_EVENT_LIST_STRUCT {
  tTS_IB_ASYNC_EVENT_RECORD_STRUCT record;
  struct list_head                 list;
};
...
int tsIbAsyncEventHandlerRegister(
  ...
  tTS_IB_ASYNC_EVENT_HANDLER handler;
  int ret;
  unsigned long flags;

  TS_IB_CHECK_MAGIC(record->device, DEVICE);

  if (record->event < 0 || record->event >= TS_IB_NUM_ASYNC_EVENT) {
  ...
  case QP:
    {
      tTS_IB_QP qp = record->modifier.qp;
  ...
  case CQ:
    {
      tTS_IB_CQ cq = record->modifier.cq;
  ...
  case EEC:
    TS_REPORT_WARN(MOD_KERNEL_IB,
                   "Async events for EECs not supported");
    ret = -EINVAL;
    goto error;
  ...
  case NONE:
    {
      tTS_IB_DEVICE_PRIVATE priv = ((tTS_IB_DEVICE) record->device)->core;
...
int tsIbAsyncEventHandlerDeregister(
  ...
  tTS_IB_ASYNC_EVENT_HANDLER handler = handle;
  unsigned long flags;
...
void tsIbAsyncEventDispatch(
  ...
  tTS_IB_ASYNC_EVENT_LIST event;
  tTS_IB_DEVICE_PRIVATE priv = ((tTS_IB_DEVICE) event_record->device)->core;
...
void tsIbAsyncThread(
  ...
  event = list_entry(entry, tTS_IB_ASYNC_EVENT_LIST_STRUCT, list);
  priv  = ((tTS_IB_DEVICE) event->record.device)->core;

  switch (event_table[event->record.event].mod) {
  case QP:
    sprintf(mod_buf, " (QP %p)", event->record.modifier.qp);
    handler_list = &((tTS_IB_QP) event->record.modifier.qp)->async_handler_list;
    handler_lock = &((tTS_IB_QP) event->record.modifier.qp)->async_handler_lock;
    break;

  case CQ:
    sprintf(mod_buf, " (CQ %p)", event->record.modifier.cq);
    handler_list = &((tTS_IB_CQ) event->record.modifier.cq)->async_handler_list;
    handler_lock = &((tTS_IB_CQ) event->record.modifier.cq)->async_handler_lock;
    break;
  ...
  case PORT:
    ...
    tsIbCacheUpdate(event->record.device, event->record.modifier.port);
    break;
  ...
  TS_TRACE(MOD_KERNEL_IB, T_VERY_VERBOSE, TRACE_KERNEL_IB_GEN,
           ...
           ((tTS_IB_DEVICE) event->record.device)->name,
           mod_buf);
  ...
  list_for_each_safe(pos, n, handler_list) {
    handler = list_entry(pos, tTS_IB_ASYNC_EVENT_HANDLER_STRUCT, list);
