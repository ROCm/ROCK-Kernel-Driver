...
static void* eq_poll_thread(void* eq_pollt_ptr)
{
  ...
  if ( EVAPI_register_thread() != VAPI_OK ) {
    MTL_ERROR1(MT_FLFMT("%s(pid="MT_ULONG_PTR_FMT"): FS_register_thread failed\n"), __func__, MOSAL_getpid());
    return NULL;
  }
  ...
  eq_pollt_p->state= EQ_POLLT_EXIT_DONE;
  if ( EVAPI_deregister_thread() != VAPI_OK ) {
    MTL_ERROR1(MT_FLFMT("%s(pid="MT_ULONG_PTR_FMT"): FS_deregister_thread failed\n"), __func__, MOSAL_getpid());
  }
  return (void*)VIP_OK;
}
...
#define REMOVE_FROM_REV_INFO_DB(hobul_p,obj_vipkl_hndl,obj_type) {        \
  ...
  vret = VIP_hashp_erase(hobul_p->obj_type##_rev_info_db,                 \
                    ...
                    &val);                                                \
  if (vret != VIP_OK) {                                                   \
    return VAPI_INVAL_HNDL;                                               \
  }                                                                       \
  return (VAPI_##obj_type##_hndl_t) val;                                  \
}
static inline VAPI_pd_hndl_t remove_from_pd_rev_info_db(HOBUL_t *hobul_p, PDM_pd_hndl_t pd_hndl)
