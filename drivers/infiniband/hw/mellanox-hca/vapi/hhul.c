...
)
{
  ...
  if (vendor_id == MT_MELLANOX_IEEE_VENDOR_ID) {
      switch(device_id) {
#if !defined(NOTAVOR)
        case MT23108_DEV_ID:  /* Tavor (InfiniHost) */
           rc = THHUL_hob_create(hca_ul_resources_p, hhul_hca_hndl_p);
           break;
#endif
      /* Add other Mellanox devices here */
      default:
          return HH_ENOSYS; /* Unknown device */
      }
  } else { /* unknown vendor */
    return HH_ENOSYS;
  }
