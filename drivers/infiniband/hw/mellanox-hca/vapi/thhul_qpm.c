...
{ 
  ...
  if (cur_state == VAPI_RESET) {  
    ...
    THHUL_cqm_cq_cleanup(qp->rq_cq, qp->qpn, qpm->srqm, qp->srq);
    MOSAL_spinlock_dpc_lock(&(qp->rq_res.q_lock));
  ...
  if (cur_state == VAPI_RESET) {  
    ...
    if (qp->sq_cq != qp->rq_cq) /* additional cleaning required only if SQ's CQ is different */
      THHUL_cqm_cq_cleanup(qp->sq_cq, qp->qpn, qpm->srqm, HHUL_INVAL_SRQ_HNDL);
    MOSAL_spinlock_dpc_lock(&(qp->sq_res.q_lock));
...
{ 
  ...
  MOSAL_spinlock_dpc_lock(&(qp->sq_res.q_lock)); /* protect wqe_draft and WQE allocation/link */
...
)
{
  ...
  MOSAL_spinlock_dpc_lock(&(qp->sq_res.q_lock)); /* protect wqe_draft as well as WQE allocation/link */
...
{ 
  ...
  MOSAL_spinlock_dpc_lock(&(qp->rq_res.q_lock)); /* protect wqe_draft as well as WQE allocation/link */
...
{ 
  ...
  *qp_ts_type_p= qp->ts_type;
  ...
  if (associated_q != NULL) { /* Release WQEs (if not from SRQ) */
    MOSAL_spinlock_dpc_lock(&(associated_q->q_lock));
    *wqes_released_p= 
