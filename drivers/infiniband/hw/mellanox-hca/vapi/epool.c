...
# undef MOSAL_spinlock_dpc_lock
# undef MOSAL_spinlock_unlock
# define MOSAL_spinlock_dpc_lock(p)  0
# define MOSAL_spinlock_unlock(p)
# define ELSE_ERROR(f)
#else
# define ELSE_ERROR(f)  else { MTL_ERROR1("%s MOSAL_spinlock_dpc_lock failed\n", f); }
#endif
...
{
  Index  p = EPOOL_NULL;
  int    acq = MOSAL_spinlock_dpc_lock(&lst->spinlock);
