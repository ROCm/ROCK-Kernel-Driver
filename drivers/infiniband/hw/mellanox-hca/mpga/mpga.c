                        ...
                        u_int16_t *header_size_p, void **header_buf_p)
{
    ...
    if ((temp_header_buff = ALLOCATE(u_int16_t,(header_size)/sizeof(u_int16_t)))== NULL)
    ...
    temp_header_buff += header_size/sizeof(u_int16_t); /* Building the header from end to start */ 
                         ...
                         u_int16_t *header_size_p, void **header_buf_p)
{
    ...
    if ((temp_header_buff = ALLOCATE(u_int16_t,(header_size)))== NULL)
    { /* Allocting size in bytes*/
        MTL_TRACE('5', "\nfailed to allocate temp_buffer_p");
        return(MT_ENOMEM);
    };
    ...
    temp_header_buff += header_size/sizeof(u_int16_t); 
