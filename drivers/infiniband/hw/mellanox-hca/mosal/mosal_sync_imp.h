...
{
#if 0
  /* this change is to solve quickly problems that occured in kernel 2.6.x
     We should re-think on what kind of locks we use in each case */
  spin_lock_bh(&sp->lock);
  sp->level = SPL_DPC_SAFE;
  return MT_OK;
#else
  return MOSAL_spinlock_irq_lock(sp);
#endif
}
...
static inline void MOSAL_spinlock_unlock(MOSAL_spinlock_t *sp)
{
  switch ( sp->level ) {
    ...
    case SPL_IRQ_SAFE:
      spin_unlock_irqrestore(&sp->lock, sp->flags);
      break;
    default:
