...
{
#if !defined(TS_MOSAL_CHECK_NR_FREE_PAGES)
  return 1;
#else
...
int find_m_lock_unlock_ptrs(sys_lock_ptr_t *lock_p, sys_lock_ptr_t *unlock_p)
{
  ...
  *lock_p = *unlock_p = 0;
  ...
  return 0;
#elif defined(sys_call_table) || defined(USE_SYS_CALL_TABLE)
  MTL_DEBUG4("%s: Warning, directly accessing sys_call_table will become \
                  obsolete in the future.\n", __func__ );
  *lock_p = (sys_lock_ptr_t)sys_call_table[SYS_mlock]; 
  *unlock_p = (sys_lock_ptr_t)sys_call_table[SYS_munlock]; 
  return 0;
#else
  unsigned long **uts_p = (unsigned long **)&system_utsname;
  u_int32_t i;

  for ( i=0; i<1000000; ++i, ++uts_p ) {
#if defined(__i386__)
    if ( uts_p[3]==(unsigned long *)sys_read &&
         uts_p[4]==(unsigned long *)sys_write &&
         uts_p[6]==(unsigned long *)sys_close &&
         uts_p[61]==(unsigned long *)sys_chroot &&
         uts_p[114]==(unsigned long *)sys_wait4 ) {
      *lock_p = (sys_lock_ptr_t)uts_p[150];
      *unlock_p = (sys_lock_ptr_t)uts_p[151];
      return 0;
    }
#elif defined(__x86_64__)
    if ( uts_p[0]==(unsigned long *)sys_read &&
         uts_p[1]==(unsigned long *)sys_write &&
         uts_p[3]==(unsigned long *)sys_close &&
         uts_p[161]==(unsigned long *)sys_chroot &&
         uts_p[61]==(unsigned long *)sys_wait4 ) {
      *lock_p = (sys_lock_ptr_t)uts_p[149];
      *unlock_p = (sys_lock_ptr_t)uts_p[150];
      return 0;
    }
#else
    if ( uts_p[0]==(unsigned long *)sys_readv && 
         uts_p[1]==(unsigned long *)sys_writev &&
         uts_p[2]==(unsigned long *)sys_getsid &&
         uts_p[3]==(unsigned long *)sys_fdatasync ) {
      *lock_p = (sys_lock_ptr_t)uts_p[5];
      *unlock_p = (sys_lock_ptr_t)uts_p[6];
      return 0;
    }
#endif
  }
 ...
 *  should_lock
 */
static int should_lock(struct vm_area_struct *vma)
{
 ...
 *  should_unlock
 */
static int should_unlock(struct vm_area_struct *vma)
{
  if (
       (vma->vm_flags&VM_IO)
#ifdef VM_NO_UNLOCK
