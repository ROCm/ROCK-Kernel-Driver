...
call_result_t MOSAL_PCI_get_cfg_hdr(u_int8_t bus, u_int8_t dev_func, MOSAL_PCI_cfg_hdr_t * cfg_hdr)
{
    call_result_t ret;
    ...
    {
        ret = MOSAL_PCI_read_config_dword(bus,dev_func, offset, &dword);
        if (ret != MT_OK)
        {
