...
/* sync on modify qp */
#define QP_MODIFY_BUSY_ON(qp_p)     MOSAL_spinlock_dpc_lock(&qp_p->mod_lock);    \
                                    if (qp_p->mod_busy) {   \
...
#define QP_MODIFY_BUSY_OFF(qp_p)    MOSAL_spinlock_dpc_lock(&qp_p->mod_lock);    \
                                    qp_p->mod_busy = FALSE;    \
                                    MOSAL_spinlock_unlock(&qp_p->mod_lock);

#define QPM_MCG_BUSY_ON(qp_p)       MOSAL_spinlock_dpc_lock(&qp_p->mcg_lock);    \
                                    if (qp_p->mcg_busy) {   \
...
#define QPM_MCG_BUSY_OFF(qp_p)      MOSAL_spinlock_dpc_lock(&qp_p->mcg_lock);    \
                                    qp_p->mcg_busy = FALSE;    \
                                    MOSAL_spinlock_unlock(&qp_p->mcg_lock);
...
#define CHECK_VRET(rfunc,vret)  if (vret != VIP_OK) {                                  \
                                  ...
                                  __func__,rfunc, vret, VAPI_strerror(vret));      \
                                }
