...
int vipkl_dev_release(struct inode *inode, struct file *filp)
{
	int i;
  sigset_t mask;
  VIPKL_linux_priv_t *lp = (VIPKL_linux_priv_t *)filp->private_data;
                          ...
                          unsigned int ioctl_num, unsigned long ioctl_param)
{
    ...
    VIPKL_linux_priv_t *lp = (VIPKL_linux_priv_t *)file->private_data;
...
#define MAX_PO_SZ		(8192*2)    
    ...
    switch ( ioctl_num ) {
      case VIPKL_IOCTL_CALL:
        rc = VIPKL_ioctl(vip_ioctl_wrap.ops, lp->rsct_arr, pi, vip_ioctl_wrap.pi_sz, po,vip_ioctl_wrap.po_sz, &bytes_to_return);
        break;
      case FS_IOCTL_CALL:
        MTL_DEBUG1(MT_FLFMT("%s(pid="MT_PID_FMT"): ops=%d, file=%p, lp=%p"), __func__, MOSAL_getpid(), vip_ioctl_wrap.ops, file, lp);
        rc = FS_ioctl(vip_ioctl_wrap.ops, file, lp, pi, vip_ioctl_wrap.pi_sz, po,vip_ioctl_wrap.po_sz, &bytes_to_return);
        break;
      default:
        rc = VIP_ENOSYS;
    }
