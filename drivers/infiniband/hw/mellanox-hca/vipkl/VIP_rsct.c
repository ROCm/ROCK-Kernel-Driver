...
VIP_ret_t VIP_RSCT_deregister_rsc(VIP_RSCT_t usr_ctx,VIP_RSCT_rscinfo_t* rsc_p, VIP_RSCT_rsctype_t rsc_type)
{
    ...
    MOSAL_spinlock_lock(&usr_ctx->rsc_lock_array[rsc_type]);
        ...
        if ( rsc_p == usr_ctx->iter) {
          usr_ctx->iter = rsc_p->next;
        }
    MOSAL_spinlock_unlock(&usr_ctx->rsc_lock_array[rsc_type]);
...
#define GET_RSC_HNDL(rsc_hndl, rsc_type, rsc_hndl_p, rc) \
  rc = VIP_OK; \
  switch ( rsc_type ) { \
    case VIP_RSCT_EQ: \
      *((VIPKL_EQ_hndl_t *)rsc_hndl_p) = rsc_hndl.rsc_eq_hndl; \
      break; \
    case VIP_RSCT_CQBLK: \
      *((VIPKL_cqblk_hndl_t *)rsc_hndl_p) = rsc_hndl.rsc_cqblk_hndl; \
      break; \
/*    case VIP_RSCT_MW: \
      *((IB_rkey_t *)rsc_hndl_p) = rsc_hndl.rsc_mw_hndl; \
      break; */ \
    case VIP_RSCT_QP: \
      *((QPM_qp_hndl_t *)rsc_hndl_p) = rsc_hndl.rsc_qp_hndl; \
      break; \
    case VIP_RSCT_CQ: \
      *((CQM_cq_hndl_t *)rsc_hndl_p) = rsc_hndl.rsc_cq_hndl; \
      break; \
    case VIP_RSCT_MR: \
      *((MM_mrw_hndl_t *)rsc_hndl_p) = rsc_hndl.rsc_mr_hndl; \
      break; \
/*    case VIP_RSCT_FMR: \
      *(( *)rsc_hndl_p) = rsc_hndl.; \
      break; */ \
    case VIP_RSCT_PD: \
      *((PDM_pd_hndl_t *)rsc_hndl_p) = rsc_hndl.rsc_pd_hndl; \
      break; \
    case VIP_RSCT_DEVMEM: \
      *((DEVMM_dm_hndl_t *)rsc_hndl_p) = rsc_hndl.rsc_devmem_hndl; \
      break; \
    default: \
      rc = VIP_EINVAL_PARAM; \
  }
...
{
  if ( usr_ctx == VIP_RSCT_NULL_USR_CTX ) {
    MTL_DEBUG1(MT_FLFMT("%s: usr_ctx=NULL"), __func__);
    return VIP_ENOSYS;
  }

  if ( usr_ctx == VIP_RSCT_IGNORE_CTX ) {
    MTL_DEBUG1(MT_FLFMT("%s: failed SANITY CHECK: trying to close hca while resources are still allocated"), __func__);
    return VIP_ENOSYS;
  }

  MOSAL_spinlock_lock(&usr_ctx->rsc_lock_array[rsc_type]);
  usr_ctx->iter = usr_ctx->rsc_array[rsc_type];
  MOSAL_spinlock_unlock(&usr_ctx->rsc_lock_array[rsc_type]);
...
VIP_ret_t VIP_RSCT_get_cur_rsrc(VIP_RSCT_t usr_ctx, VIP_RSCT_rsctype_t rsc_type, void *rsc_hndl_p)
{
  ...
  MOSAL_spinlock_lock(&usr_ctx->rsc_lock_array[rsc_type]);
  if ( usr_ctx->iter ) {
    GET_RSC_HNDL(usr_ctx->iter->rsc_hndl, rsc_type, rsc_hndl_p, rc)
  }
  MOSAL_spinlock_unlock(&usr_ctx->rsc_lock_array[rsc_type]);
  return rc;
}
