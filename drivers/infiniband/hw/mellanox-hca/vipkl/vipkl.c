                            ...
                            MT_u_int_t *act_p)
{
  ...
  for ( act=0, rc=VIP_OK;
        ((*hca_hndl_p)<VIP_MAX_HCA) && count && (rc==VIP_OK);
        ++(*hca_hndl_p), count-=act, addr_lst_p+=count ) {
    if ( VIP_array_find_hold(hobs, *hca_hndl_p, (VIP_array_obj_t *)&hob) == VIP_OK ) {
      MTL_DEBUG1(MT_FLFMT("%s: hca_hndl=%d"), __func__, *hca_hndl_p);
      rc = MM_get_mr_list(usr_ctx, HOBKL_get_mm(hob), *hca_hndl_p, addr_lst_p, count, &act);
      VIP_array_find_release(hobs, *hca_hndl_p);
    }
  }
