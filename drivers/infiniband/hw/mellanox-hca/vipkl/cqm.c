                         ...
                         MT_bool in_rsct_cleanup)
{
  ...
  if ( ret != VIP_OK ) {
    MTL_ERROR1(MT_FLFMT("failed VIP_array_erase_prep: got %s handle=%d"),VAPI_strerror_sym(ret),cq_hndl);
    return ret;
  }
    ...
    )
{
  ...
  if ( ret != VIP_OK ) {
    MTL_ERROR5(MT_FLFMT("%s: Failed VIP_array_find_hold for cq_hndl=0x%X (%s)"),__func__,
               cq_hndl,VAPI_strerror_sym((ret == VIP_EINVAL_HNDL) ? VIP_EINVAL_CQ_HNDL : ret));
...
CQM_suspend_cq (VIP_RSCT_t usr_ctx,CQM_hndl_t cqm_hndl, CQM_cq_hndl_t cq_hndl, MT_bool do_suspend)
{
    ...
    ret = VIP_array_find_hold(cqm_hndl->array, cq_hndl, (VIP_array_obj_t *)&cq);
    if ( ret!=VIP_OK ) {
      MTL_ERROR1(MT_FLFMT("%s: invalid cq handle=0x%x (%s)"),__func__,cq_hndl,VAPI_strerror_sym(ret));
      return VIP_EINVAL_CQ_HNDL;
    }
