...
#define MMU_BUSY_ON(mm_p,t)     MOSAL_spinlock_dpc_lock(&(mm_p)->t##_lock);    \
                                if ((mm_p)->t##_busy) {                        \
...
#define MMU_BUSY_OFF(mm_p,t)    MOSAL_spinlock_dpc_lock(&(mm_p)->t##_lock);    \
                                (mm_p)->t##_busy = FALSE;                      \
                                MOSAL_spinlock_unlock(&(mm_p)->t##_lock);
                                    ...
                                    VAPI_size_t sz, VAPI_mrw_acl_t acl, MOSAL_iobuf_t *iobuf_p)
{
  ...
  rr = MOSAL_iobuf_register(start, sz, prot_ctx, mosal_perm, iobuf_p, MOSAL_IOBUF_LNX_FLG_MARK_FULL_PAGES_DONTCOPY);
    if ( rr != MT_OK ) {
...
VIP_ret_t MM_create_mw(VIP_RSCT_t usr_ctx,MM_hndl_t mm_h,PDM_pd_hndl_t pdm_pd_h,IB_rkey_t *r_key_p)
{
	...
	if( rc != VIP_OK ) 
	{
		MTL_ERROR1(MT_FLFMT("%s: failed inserting mwh to array (%d:%s)"),__func__,rc,VAPI_strerror_sym(rc));
	...
	if ( rc != VIP_OK ) {
		MTL_ERROR1(MT_FLFMT("%s: ERROR: VIP_hash_insert() (%d:%s)"),__func__,rc,VAPI_strerror_sym(rc)); 
...
create_mw_alloc_err:
	...
	if (hhrc != HH_OK)
	{
		MTL_ERROR1(MT_FLFMT("%s: HH_free_mw failed (%d:%s)"),__func__, rc, VAPI_strerror_sym(rc));
	}
...
VIP_ret_t MM_destroy_mr(VIP_RSCT_t usr_ctx,MM_hndl_t mm_hndl,MM_mrw_hndl_t mr_hndl)
{
    ...
    if ( rc != VIP_OK ) 
    {
            MTL_ERROR1(MT_FLFMT("%s: invalid mr hndl: 0x%x (%d:%s)"),
                       __func__,mr_hndl,rc,VAPI_strerror_sym(rc));
            MT_RETURN(VIP_EINVAL_MR_HNDL);
    }
    mm_mro_p = (MM_mro*)val;    
    rc = VIP_RSCT_check_usr_ctx(usr_ctx,&mm_mro_p->rsc_ctx);
                         ...
                         MT_u_int_t *act_p)
{
  VIP_ret_t rc;
  MT_u_int_t act;
  MM_mrw_hndl_t mr_hndl;
  MM_mro *mro_p;
  call_result_t mrc;
  MOSAL_iobuf_props_t iobuf_props;

  MTL_DEBUG1(MT_FLFMT("%s(pid="MT_PID_FMT"): addr_lst_p=%p"), __func__, MOSAL_getpid(), addr_lst_p);
  /* since addr_lst_p = NULL siginifies that we just want to walk
     through the list of MRs and we don't need to return VIP_region_data_t
     objects we can set a large value for count and make sure that it
     happens in one call */
  if ( addr_lst_p == NULL ) {
    count = (MT_u_int_t)(-1); // max
  }
  for ( act=0, rc=VIP_RSCT_get_cur_rsrc(usr_ctx, VIP_RSCT_MR, &mr_hndl);
        (act<count) && (rc==VIP_OK);
        rc=VIP_RSCT_get_next_rsrc(usr_ctx, VIP_RSCT_MR, &mr_hndl) ) {
    rc = VIP_array_find_hold(mmu->mr_by_hndl, mr_hndl, (VIP_array_obj_t *)&mro_p);
    if ( rc != VIP_OK ) {
      rc = VIP_ERROR;
      break;
    }

    /* VAPI_MPR types do not have an iobuf object */
    if ( mro_p->iobuf ) {
      mrc = MOSAL_iobuf_get_props(mro_p->iobuf, &iobuf_props);
      if ( mrc != MT_OK ) {
        MTL_ERROR1(MT_FLFMT("%s: fatal - cannot get iobuf props"), __func__);
        rc = VIP_ERROR;
        break;
      }
      if ( addr_lst_p == NULL ) {
        mrc = MOSAL_iobuf_restore_perm(mro_p->iobuf);
        if ( mrc != MT_OK ) {
          MTL_ERROR1(MT_FLFMT("%s: fatal - cannot get iobuf props"), __func__);
          rc = VIP_ERROR;
          break;
        ...
        if ( !(iobuf_props.os_dep_flags & MOSAL_IOBUF_LNX_FLG_PROP_MARKED_DONT_COPY) ) {
          addr_lst_p[act].start_addr = iobuf_props.va;
          addr_lst_p[act].size = iobuf_props.size;
          addr_lst_p[act].page_shift = iobuf_props.page_shift;
          act++;
          MTL_DEBUG1(MT_FLFMT("%s: pid="MT_ULONG_PTR_FMT", va="VIRT_ADDR_FMT", size="SIZE_T_FMT), __func__, MOSAL_getpid(), iobuf_props.va, iobuf_props.size);
        }
      }
    }

    VIP_array_find_release(mmu->mr_by_hndl, mr_hndl);
  }
  
  if ( (act<count) && (rc==VIP_OK) ) {
    rc = VIP_EOL;
  }
