...
/*************************************************************************
 * Function: MM_get_mr_list
 *
 * Arguments:
 *  usr_ctx(in) user context
 *  mmu(in) mmu handle
 *  hca_hndl(in) hca handle
 *  addr_lst_p(out) array of VIP_region_data_t to return to called
 *  count(in) size of array
 *  act_p(out) number of elements returned
 *
 * Returns:
 *  VIP_OK
 *  VIP_EOL not all elements in the array are filled
 *
 * Description:
 *  1. The function returns data on all the memory regions allocated for the process
 *     which will be inheritted by child processes
 *  2. If addr_lst_p is NULL then it restores the permissions (in the ptes)
 *     for all memory region
 *
 *************************************************************************/ 
VIP_ret_t MM_get_mr_list(VIP_RSCT_t usr_ctx,
                         MM_hndl_t mmu,
