...
typedef struct THHUL_pd_st {
    ...
    /* counter and spinlock for tracking allocated avs for this pd */
    u_int32_t         num_allocated_avs;
    MOSAL_spinlock_t  av_ctr_sl; 

    /* fields for linked list of all PDs which contain 1 or more allocated udavs */
    struct THHUL_pd_st *next_pd_with_avs;
    MOSAL_spinlock_t   sl_pd_ll_element;
} THHUL_pd_t;
...
struct THHUL_pdm_st {
    ...
    /* head of linked list of all PDs which contain allocated udavs */
    struct THHUL_pd_st *first_pd_with_avs;
    MOSAL_spinlock_t   sl_pd_ll_head;
} ;
