/*
  ...
  $Id: mad_mem_compat.h,v 1.6 2004/02/25 00:35:21 roland Exp $
*/
...
static inline uintptr_t tsIbMadBufferAddress(
                                             void *buf
                                             ) {
  return (uintptr_t) buf;
}

static inline int tsIbMadRegisterMemory(
                                        tTS_IB_PD_HANDLE  pd,
                                        tTS_IB_MR_HANDLE *mr,
                                        tTS_IB_LKEY      *lkey
                                        ) {
    ...
    result = tsIbMemoryRegisterPhysical(pd,
                                        &buffer_list,
                                        1, /* list_len */
                                        &iova,
                                        buffer_list.size,
    ...
    if (result) {
      TS_REPORT_WARN(MOD_KERNEL_IB,
                     "tsIbMemoryRegisterPhysical failed "
