/*
  ...
  $Id: sdp_conn.c,v 1.69 2004/02/24 23:48:47 roland Exp $
*/
...
)
{
  ...
  *end_index = 0;
  ...
  if (start_index < _dev_root_s.sk_size) {
	...
	d_guid = TS_GW_SWAP_64(*(tUINT64 *)(conn->d_gid + sizeof(tUINT64)));
	s_guid = TS_GW_SWAP_64(*(tUINT64 *)(conn->s_gid + sizeof(tUINT64)));
  ...
  for (hca = _dev_root_s.hca_list, hca_count = 0;
    ...
    for (port = hca->port_list, port_count = 0;
	 ...
	 port = port->next, port_count++) {
      ...
      subnet_prefix = TS_GW_SWAP_64(*(tUINT64 *)(port->gid));
      guid = TS_GW_SWAP_64(*(tUINT64 *)(port->gid + sizeof(tUINT64)));
