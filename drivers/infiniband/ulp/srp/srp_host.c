/*
  ...
  $Id: srp_host.c,v 1.3 2004/02/25 01:13:43 roland Exp $
*/
...
void srp_fmr_flush_function( tTS_IB_FMR_POOL_HANDLE fmr_pool, void *flush_arg )
{
    ...
    TS_REPORT_DATA( MOD_SRPTP, "FMR flush completed for hca %d, retrying %d I/Os",
                    hca->hca_index+1, retry_count );
    ...
    list_for_each_safe( ioq_entry, temp_entry, &hca->resource_list ) {
        ...
        /*
         * Only retry I/Os that were present when the flush function was invoked
         * If the FMR pool is grossly undersized, there maybe enough I/Os to use
         * all the fmr entries and recursively add back to the resource queue.  In which, case
         * we will be stuck in a loop, unless be break here.
         */
        if ( --retry_count == 0 )
            break;
    }
...
void srp_pending_to_scsi( srp_target_t *target, unsigned long status )
{
    ...
    list_for_each_safe( ioq_entry, temp_entry, &target->pending_list ) {
        ...
        if ( ioq->type == IOQ_COMMAND ) {
            scsi_cmnd->result = status << 16;

            TS_REPORT_STAGE( MOD_SRPTP,
                             " Sending IO back to scsi from pending list SCpnt %p ioq %p",
                             scsi_cmnd, ioq );
...
void srp_move_to_pending( srp_target_t *target )
{
    ...
    spin_lock_irqsave( &target->spin_lock, cpu_flags1 );
    ...
    spin_lock_irqsave( &hca->spin_lock, cpu_flags2 );
    ...
    list_for_each_safe( ioq_entry, temp_entry, &hca->resource_list ) {
        ...
        if ( ioq->target == target ) {
             ...
             */
            spin_unlock_irqrestore( &target->spin_lock, cpu_flags1 );
    ...
    spin_unlock_irqrestore( &hca->spin_lock, cpu_flags2 );
...
srp_process_resp (srp_target_t *target, srp_pkt_t * pkt)
{
    ...
    if (ioq->req == NULL) {
		...
		if ( ioq->queue_type == QUEUE_ACTIVE ) {
            ...
            TS_REPORT_FATAL( MOD_SRPTP, "Processing a response on wrong queue %p ioq" );
            spin_unlock_irqrestore(&target->spin_lock, cpu_flags);
            return 0;
        }
        ...
        if ( next_ioq ) {
            TS_REPORT_DATA( MOD_SRPTP, "Retrying ioq %p for target %d from target resource q",
                            next_ioq, s->target->target_index );
...
//    sr_list->data = st_buffer[0].address;
    ...
    (uintptr_t) srp_remote_buf = (uintptr_t) (srp_pkt->data) + sizeof (srp_cmd_t);
...
{
    return ("SRP HOST  Adapter: $Id: srp_host.c,v 1.3 2004/02/25 01:13:43 roland Exp $");
}
...
{
    if (s->state != SRP_HOST_LOGOUT_INPROGRESS) {
    	TS_REPORT_STAGE( MOD_SRPTP, "target disconnected us %p in state %x", s, s->state );
