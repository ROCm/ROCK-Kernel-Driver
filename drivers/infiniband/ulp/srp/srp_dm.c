/*
  ...
  $Id: srp_dm.c,v 1.2 2004/02/24 21:49:07 roland Exp $
*/
...
int srp_dm_query( srp_host_port_params_t *port )
{
    ...
    if ( port->port_state == TS_IB_PORT_STATE_ACTIVE ) {
		...
		/*
		 * Workaround for MAD problems
		 * Delay the connections thro the second port
		 */
		TS_REPORT_STAGE( MOD_SRPTP,
                         "Delaying DM query thru hca %d local port %d",
                         port->hca->hca_index+1, port->local_port );
    ...
    switch (event->event) {
		...
		case	TS_IB_PORT_ERROR :
			...
			for ( i = 0; i < max_srp_targets ; i++ ) {
                ...
                srp_host_conn_t	*conn = srp_targets[i].active_conn;

				if ( ( conn ) && ( conn->state == SRP_UP ) &&
                     ( conn->port == port ) )  {
                    TS_REPORT_STAGE(MOD_SRPTP,
						" Target %d active conn thru port %d active_count %d",
						i, conn->port->local_port,
						srp_targets[i].active_count );
