/*
  ...
  $Id: ip2pr_link.c,v 1.25 2004/02/25 00:38:28 roland Exp $
*/
  ...
  for (path_elmt = _tsIp2prLinkRoot.path_list, counter = 0;
    ...
    if (!(start_index > counter)) {
      ...
      offset += sprintf((buffer + offset),
			...
			(unsigned long long)TS_GW_SWAP_64(*(tUINT64 *)path_elmt->path_s.dgid),
			(unsigned long long)TS_GW_SWAP_64(*(tUINT64 *)(path_elmt->path_s.dgid +
                                                                        sizeof(tUINT64))),
...
)
{
  ...
  case 0:
    ...
    TS_TRACE(MOD_IP2PR, T_VERY_VERBOSE, TRACE_FLOW_INOUT,
	     ...
	     TS_GW_SWAP_64(*(tUINT64 *)path->dgid),
...
)
{
   ...
   */
  if (TS_IB_PORT_ACTIVE == record->event) {
    port_state = TS_IB_PORT_STATE_ACTIVE;
  } else {
    port_state = TS_IB_PORT_STATE_DOWN;
  }
  ...
  *gid_node = NULL;
  ...
  for (sgid_elmt = _tsIp2prLinkRoot.src_gid_list;
    ...
    if ((TS_IB_PORT_STATE_ACTIVE == sgid_elmt->port_state) &&
        (0 == memcmp(sgid_elmt->gid, src_gid, sizeof (tTS_IB_GID)))) {
      *gid_node = sgid_elmt;
...
)
{
  ...
  sgid_elmt->port = port;
  sgid_elmt->port_state = TS_IB_PORT_STATE_ACTIVE;
