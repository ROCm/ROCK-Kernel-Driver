/*
  ...
  $Id: ipoib_main.c,v 1.100 2004/02/25 01:11:29 roland Exp $
*/
...
int __init tsIpoIbDriverAddPort(
                                const char *format,
  ...
  if (result < 0) {
    TS_REPORT_FATAL(MOD_IB_NET,
                    "failed to setu port monitor for device %d, port %d (ret = %d)",
                    hca, port, result);
...
static int __init tsIpoibDriverInitModule(
                                          void
                                          ) {
  int ret;

  ret = tsIpoibTransportCreateDevices();
  if (ret) {
    return ret;
  }
...
static void __exit tsIpoibDriverCleanupModule(
                                              void
                                              ) {
  struct list_head *ptr, *tmp;
...
module_init(tsIpoibDriverInitModule);
module_exit(tsIpoibDriverCleanupModule);
