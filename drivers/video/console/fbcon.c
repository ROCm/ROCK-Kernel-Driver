...
void accel_cursor(struct vc_data *vc, struct fb_info *info, int flags, int xx, int yy)
{
	unsigned short charmask = vc->vc_hi_font_mask ? 0x1ff : 0xff;
	int bgshift = (vc->vc_hi_font_mask) ? 13 : 12;
	int fgshift = (vc->vc_hi_font_mask) ? 9 : 8;
...
static void fbcon_cursor(struct vc_data *vc, int mode)
{
	...
	case CM_ERASE:
		if (cursor_drawn)
			accel_cursor(vc, info, 0, vc->vc_x,
				  	real_y(p, y));
		cursor_drawn = 0;
		break;
	case CM_MOVE:
	case CM_DRAW:
		if (cursor_drawn)
			accel_cursor(vc, info, FB_CUR_SETCUR, vc->vc_x,
					  real_y(p, y));
...
static void fb_vbl_handler(int irq, void *dev_id, struct pt_regs *fp)
{
	...
	if (vbl_cursor_cnt && --vbl_cursor_cnt == 0) {
		...
		if (!cursor_drawn)
			flag = FB_CUR_SETCUR;
		accel_cursor(vc, info, flag, vc->vc_x, real_y(p, vc->vc_y));
		cursor_drawn ^= 1;
