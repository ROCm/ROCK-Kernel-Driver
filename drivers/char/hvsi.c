...
static int hvsi_version_respond(struct hvsi_struct *hp, uint16_t query_seqno)
{
	...
	wrote = hvterm_put_chars(hp->vtermno, (char *)&packet, packet.len);
	if (wrote != packet.len) {
...
static irqreturn_t hvsi_interrupt(int irq, void *arg, struct pt_regs *regs)
{
	...
	while (again) {
		...
		if (hp->state == HVSI_CLOSED) {
			/* we could get a CLOSED message from the FSP even when we don't
...
static int poll_for_state(struct hvsi_struct *hp, int state)
{
...
static int hvsi_query(struct hvsi_struct *hp, uint16_t verb)
{
	...
	wrote = hvterm_put_chars(hp->vtermno, (char *)&packet, packet.len);
	if (wrote != packet.len) {
		printk(KERN_ERR "%s: couldn't send query (%i)!\n", __FUNCTION__, wrote);
		return -EIO;
	}
...
static int hvsi_put_chars(struct hvsi_struct *hp, const char *buf, int count)
{
	...
	if (count > HVSI_MAX_OUTGOING_DATA) {
		/* XXX loop around hvterm_put_chars() with a bigger packet */
	...
	ret = hvterm_put_chars(hp->vtermno, (char *)&packet, packet.len);
	if (ret == packet.len) {
...
static void hvsi_close_protocol(struct hvsi_struct *hp)
{
	...
	hvterm_put_chars(hp->vtermno, (char *)&packet, packet.len);
}
