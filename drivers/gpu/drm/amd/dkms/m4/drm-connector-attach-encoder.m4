...
AC_DEFUN([AC_AMDGPU_DRM_CONNECTOR_ATTACH_ENCODER],
	[AC_MSG_CHECKING([whether drm_connector_attach_encoder() is available])
	AC_KERNEL_TRY_COMPILE_SYMBOL([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_CONNECTOR_ATTACH_ENCODER, 1, [drm_connector_attach_encoder() is available])
	],[
		AC_MSG_RESULT(no)
	])
])
