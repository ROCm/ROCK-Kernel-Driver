...
AC_DEFUN([AC_AMDGPU_DRM_CONNECTOR_UPDATE_EDID_PROPERTY],
	[AC_MSG_CHECKING([whether drm_connector_update_edid_property() is available])
	AC_KERNEL_TRY_COMPILE_SYMBOL([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_CONNECTOR_UPDATE_EDID_PROPERTY, 1, [drm_connector_update_edid_property() is available])
	],[
		AC_MSG_RESULT(no)
	])
])
