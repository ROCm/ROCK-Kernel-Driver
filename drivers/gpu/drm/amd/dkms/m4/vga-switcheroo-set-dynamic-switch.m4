...
AC_DEFUN([AC_AMDGPU_VGA_SWITCHEROO_SET_DYNAMIC_SWITCH],
	[AC_MSG_CHECKING([whether vga_switcheroo_set_dynamic_switch() exist])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_VGA_SWITCHEROO_SET_DYNAMIC_SWITCH, 1, [vga_switcheroo_set_dynamic_switch() exist])
	], [
		AC_MSG_RESULT(no)
	])
])
