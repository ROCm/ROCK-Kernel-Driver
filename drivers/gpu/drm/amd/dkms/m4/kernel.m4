...
AC_DEFUN([AC_CONFIG_KERNEL], [
	...
	AC_AMDGPU_DRM_DEVICE_FB_HELPER
	...
	AC_AMDGPU_DMA_FENCE_FLAG_TIMESTAMP_BIT
	AC_AMDGPU_DRM_PLANE_GET_DAMAGE_CLIPS_COUNT
	AC_AMDGPU_DRM_COREDUMP_PRINTER
	...
	AC_AMDGPU_DRM_ATOMIC_HELPER_WAIT_FOR_FENCES
	AC_AMDGPU_STRUCT_DRM_CRTC
...
AC_DEFUN([AC_KERNEL_COMPILE_MODULE_IFELSE], [
	...
	AS_IF(
		[AC_TRY_COMMAND(make [$2] -C $LINUX_OBJ EXTRA_CFLAGS="-Werror -Wno-error=uninitialized -Wno-error=unused-variable -Wno-error=array-bounds" M=$PWD $kbuild_src_flag $kbuild_workaround_flag $kbuild_modpost_flag) >/dev/null && AC_TRY_COMMAND([$3])],
		[$4],
