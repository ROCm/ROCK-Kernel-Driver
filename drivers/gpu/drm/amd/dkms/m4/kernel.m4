...
AC_DEFUN([AC_CONFIG_KERNEL], [
	...
	AC_AMDGPU_CHUNK_ID_SYNCOBJ
	AC_AMDGPU_IRQ_DOMAIN
	AC_AMDGPU_INVALIDATE_RANGE_END
	...
	AC_AMDGPU_OBJS_IN_STRUCTURE_DRM_FRAMEBUFFER
	...
	AC_AMDGPU_DRM_MODE_IS_420_ONLY
	AC_AMDGPU_2ARGS_SET_CRC_SOURCE
	AC_AMDGPU_SEQUENCE_IN_STRUCT_DRM_PENDING_VBLANK_EVENT
	AC_AMDGPU_VRR_ENABLED_IN_STRUCT_DM_CRTC_STATE
	AC_AMDGPU_FOR_EACH_NEW_CRTC_IN_STATE
	AC_AMDGPU_DRM_CRTC_ACCURATE_VBLANK_COUNT
	AC_AMDGPU_DP_DPRX_FEATURE_ENUMERATION_LIST
	AC_AMDGPU_DRM_ATOMIC_HELPER_PLANE_RESET
	AC_AMDGPU_ALPHA_IN_STRUCT_DRM_PLANE_STATE
	AC_AMDGPU_HELPER_PRIVATE_IN_STRUCT_DRM_MODE_CONFIG
	AC_AMDGPU_PIXEL_BLEND_MODE_IN_STRUCT_DRM_PLANE_STATE
	AC_AMDGPU_FOR_EACH_OLDNEW_PLANE_IN_STATE
	AC_AMDGPU_PRIVATE_OBJS_IN_STRUCTURE_DRM_ATOMIC_STATE
	AC_AMDGPU_PAGEFLIP_FLAGS_IN_STRUCTURE_DRM_CRTC_STATE
	AC_AMDGPU_DRM_ATOMIC_ADD_AFFECTED_PLANES
	AC_AMDGPU_FOR_EACH_OLDNEW_CONNECTOR_IN_STATE
	AC_AMDGPU_DRM_ATOMIC_CRTC_NEEDS_MODESET
	AC_AMDGPU_MAX_BPC_AND_MAX_REQUESTED_BPC_IN_STRUCT_DRM_CONNECTOR_STATE
	AC_AMDGPU_VRR_CAPABLE_PROPERTY_IN_STRUCT_DRM_CONNECTOR
	AC_AMDGPU_DRM_DP_CEC_CORRELATION_FUNCTIONS
	AC_AMDGPU_VERIFY_CRC_SOURCE_IN_STRUCT_DRM_CRTC_FUNCS
	AC_AMDGPU_COLOR_MGMT_CHANGED_IN_STRUCT_DRM_CRTC_STATE
...
AC_DEFUN([AC_KERNEL_COMPILE_IFELSE], [
	...
	AS_IF(
		[AC_TRY_COMMAND(cp conftest.c conftest.h build && make [$2] -C $LINUX_OBJ EXTRA_CFLAGS="-Werror -Wno-error=uninitialized" M=$PWD/build $modpost_flag $kbuild_src_flag) >/dev/null && AC_TRY_COMMAND([$3])],
		[$4],
