...
AC_DEFUN([AC_CONFIG_KERNEL], [
	...
	AC_AMDGPU_DRM_ATOMIC_PLANE_ENABLING
	...
	AC_AMDGPU_DRM_VBLANK_CRTC_CONFIG
	...
	AC_AMDGPU_DRM_GEM_PRIME_HANDLE_TO_DMABUF
	AC_AMDGPU_PID_TYPE
	AC_AMDGPU_FOLLOW_PFN
	AC_AMDGPU_IMPORT_GUID
	AC_AMDGPU_DRM_DP_MST_BRANCH_GUID_T
	AC_AMDGPU_LIST_CMP_FUNC_IS_CONST_PARAM
	AC_AMDGPU_DMA_FENCE_OPS_SET_DEADLINE
	AC_AMDGPU_DRM_SHOW_FDINFO
	AC_AMDGPU_LINUX_ATOMIC_LONG_TRY_CMPXCHG
	AC_AMDGPU_LINUX_LOCAL_TRY_CMPXCHG
	AC_AMDGPU_LINUX_DEVICE_CLASS
	AC_AMDGPU_KVREALLOC
	AC_AMDGPU_DMA_BUF_IS_DYNAMIC
	AC_AMDGPU_RADIX_TREE_ITER_DELETE
	AC_AMDGPU_KFIFO_PUT
	AC_AMDGPU_KFIFO_OUT_LINEAR
	AC_AMDGPU_DRM_CLIENT_REGISTER
	AC_AMDGPU_DRM_COLOR_CTM_3X4
	AC_AMDGPU_DRM_DRIVER_GEM_PRIME_MMAP
	AC_AMDGPU_DRM_GEM_PRIME_HANDLE_TO_FD
	AC_AMDGPU_DMA_FENCE_TIMESTAMP
	AC_AMDGPU_VMA_IS_INITIAL
	AC_AMDGPU_CPUINFO_X86
	AC_AMDGPU_DMA_FENCE_IS_LATER_OR_SAME
	AC_AMDGPU_WORKQUEUE
	AC_AMDGPU_DRM_EXEC_INIT
	AC_AMDGPU_DRM_DBG_PRINTER
	...
	AC_AMDGPU_PM_RUNTIME_RESUME_AND_GET
	AC_AMDGPU_PM_RUNTIME_GET_IF_ACTIVE
	AC_AMDGPU_DRM_DP_ADD_PAYLOAD_PART2_THREE_ARGUMENTS
    AC_AMDGPU_ASSIGN_STR
	AC_AMDGPU_DRM_DP_MST_EDID_READ
	AC_AMDGPU_DRM_DP_CEC_ATTACH
	AC_AMDGPU_STRUCT_FILE_OPERATION
	AC_AMDGPU_FOLLOW_PFNMAP_START
	AC_AMDGPU_DRM_FILE
	AC_AMDGPU_DRM_DP_MST_TOPOLOGY_QUEUE_PROBE
	AC_AMDGPU_DRM_CLIENT
	AC_AMDGPU_DRM_SYNCOBJ_ADD_POINT
	AC_AMDGPU_SG_ALLOC_TABLE_FROM_PAGES_SEGMENT
	AC_AMDGPU_FIRMWARE_REQUEST_NOWARN
	AC_AMDGPU_DRM_DISPLAY_INFO_SOURCE_PHYSICAL_ADDRESS
	AC_AMDGPU_CEC_NOTIFIER_CONN_REGISTER
	AC_AMDGPU_STRUCT_CEC_CONNECTOR_INFO
	AC_AMDGPU_KFD_PEERDIRECT_SUPPORT
...
AC_DEFUN([AC_KERNEL_COMPILE_MODULE_IFELSE], [
	...
	AS_IF(
		[AC_TRY_COMMAND(make [$2] -C $LINUX_OBJ EXTRA_CFLAGS="-Werror -Wno-error=unused-variable -Wno-error=array-bounds" M=$PWD $kbuild_src_flag $kbuild_workaround_flag $kbuild_modpost_flag $kbuild_cc) >/dev/null && AC_TRY_COMMAND([$3])],
		[$4],
