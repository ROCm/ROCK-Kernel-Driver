...
AC_DEFUN([AC_AMDGPU_DRM_HDMI_VENDOR_INFOFRAME_FROM_DISPLAY_MODE],
	[AC_MSG_CHECKING([whether drm_hdmi_vendor_infoframe_from_display_mode() has p,p,p interface])
	AC_KERNEL_TRY_COMPILE_SYMBOL([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_HDMI_VENDOR_INFOFRAME_FROM_DISPLAY_MODE_P_P_P, 1, [drm_hdmi_vendor_infoframe_from_display_mode() has p,p,p interface])
	], [
		AC_MSG_RESULT(no)
	])
])
