...
AC_DEFUN([AC_AMDGPU_DRM_DRIVER_GEM_PRIME_RES_OBJ],
	[AC_MSG_CHECKING([whether drm_driver->gem_prime_res_obj() is available])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_DRIVER_GEM_PRIME_RES_OBJ, 1, [drm_driver->gem_prime_res_obj() is available])
	], [
		AC_MSG_RESULT(no)
	])
])
