...
AC_DEFUN([AC_AMDGPU_DRM_SYNCOBJ_FIND_FENCE], [
	AC_MSG_CHECKING([whether drm_syncobj_find_fence() is available])
	AC_KERNEL_CHECK_SYMBOL_EXPORT([drm_syncobj_find_fence], [drivers/gpu/drm/drm_syncobj.c], [
		...
		AC_KERNEL_TRY_COMPILE([
			...
			AC_MSG_RESULT(yes)
			AC_DEFINE(HAVE_DRM_SYNCOBJ_FIND_FENCE_5ARGS, 1, [whether drm_syncobj_find_fence() wants 5 args])
		], [
			dnl #
			...
			AC_KERNEL_TRY_COMPILE([
				...
				AC_MSG_RESULT(yes)
				AC_DEFINE(HAVE_DRM_SYNCOBJ_FIND_FENCE_4ARGS, 1, [whether drm_syncobj_find_fence() wants 4 args])
			], [
				dnl #
				...
				AC_KERNEL_TRY_COMPILE([
					...
					AC_MSG_RESULT(yes)
					AC_DEFINE(HAVE_DRM_SYNCOBJ_FIND_FENCE_3ARGS, 1, [whether drm_syncobj_find_fence() wants 3 args])
				], [
					AC_MSG_RESULT(no)
				])
			])
