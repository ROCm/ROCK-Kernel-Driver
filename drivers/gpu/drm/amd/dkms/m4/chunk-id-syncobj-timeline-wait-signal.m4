...
AC_DEFUN([AC_AMDGPU_CHUNK_ID_SYNCOBJ_TIMELINE_WAIT_SIGNAL],
	[AC_MSG_CHECKING([whether CHUNK_ID_SYNCOBJ_TIMELINE_WAIT_SIGNAL is defined])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_CHUNK_ID_SYNCOBJ_TIMELINE_WAIT_SIGNAL, 1, [whether CHUNK_ID_SYNCOBJ_TIMELINE_WAIT_SIGNAL is defined])
	],[
		AC_MSG_RESULT(no)
	])
])
