...
AC_DEFUN([AC_AMDGPU_ZONE_MANAGED_PAGES],
	...
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_ZONE_MANAGED_PAGES, 1, [zone_managed_pages() is available])
	],[
		AC_MSG_RESULT(no)
		AC_MSG_CHECKING([whether zone->managed_pages is available])
		dnl #
		...
		AC_KERNEL_TRY_COMPILE([
			...
			AC_MSG_RESULT(yes)
			AC_DEFINE(HAVE_STRUCT_ZONE_MANAGED_PAGES, 1, [zone->managed_pages is available])
		], [
			AC_MSG_RESULT(no)
		])
	])
