...
AC_DEFUN([AC_AMDGPU_RESERVATION_OBJECT_WAIT_TIMEOUT_RCU],
	[AC_MSG_CHECKING([whether reservation_object_wait_timeout_rcu() is available])
	AC_KERNEL_TRY_COMPILE([
		#include <linux/reservation.h>
	],[
		reservation_object_wait_timeout_rcu(NULL, 0, 0, 0);
	],[
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_RESERVATION_OBJECT_WAIT_TIMEOUT_RCU, 1, [reservation_object_wait_timeout_rcu() is available])
	],[
		AC_MSG_RESULT(no)
	])
])
