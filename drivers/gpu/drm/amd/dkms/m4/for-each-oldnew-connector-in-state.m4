...
dnl #
AC_DEFUN([AC_AMDGPU_FOR_EACH_OLDNEW_CONNECTOR_IN_STATE],
	[AC_MSG_CHECKING([whether for_each_oldnew_connector_in_state() is defined])
	AC_KERNEL_TRY_COMPILE([
		#include <drm/drm_atomic.h>
	],[
		#if !defined(for_each_oldnew_connector_in_state)
		#error for_each_oldnew_connector_in_state not #defined
		#endif
	],[
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_FOR_EACH_OLDNEW_CONNECTOR_IN_STATE, 1, [whether for_each_oldnew_connector_in_state() is defined])
	],[
		AC_MSG_RESULT(no)
	])
])
