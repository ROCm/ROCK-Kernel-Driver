...
AC_DEFUN([AC_AMDGPU_INVALIDATE_RANGE_START], [
	AC_MSG_CHECKING([whether invalidate_range_start() wants 2 args])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_2ARGS_INVALIDATE_RANGE_START, 1, whether invalidate_range_start() wants 2 args)
	], [
		dnl #
		...
		dnl #
		AC_MSG_RESULT(no)
		AC_MSG_CHECKING([whether invalidate_range_start() wants 5 args])
		AC_KERNEL_TRY_COMPILE([
			#include <linux/mmu_notifier.h>
		], [
			struct mmu_notifier_ops *ops = NULL;
			...
			AC_MSG_RESULT(yes)
			AC_DEFINE(HAVE_5ARGS_INVALIDATE_RANGE_START, 1, whether invalidate_range_start() wants 5 args)
		], [
			AC_MSG_RESULT(no)
		])
	])
