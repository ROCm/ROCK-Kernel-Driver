...
AC_DEFUN([AC_AMDGPU_ARCH_IO_RESERVE_FREE_MEMTYPE_WC_REAL],
	[AC_KERNEL_TRY_COMPILE_SYMBOL([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_ARCH_IO_RESERVE_FREE_MEMTYPE_WC, 1, [arch_io_{reserve/free}_memtype_wc() are available])
	], [
		AC_MSG_RESULT(no)
	])
])
...
AC_DEFUN([AC_AMDGPU_ARCH_IO_RESERVE_FREE_MEMTYPE_WC],
	AC_MSG_CHECKING([whether arch_io_{reserve/free}_memtype_wc() are available])
