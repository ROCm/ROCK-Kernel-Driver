...
AC_DEFUN([AC_AMDGPU_DRM_CONNECTOR_PUT],[
	AC_MSG_CHECKING([whether drm_connector_put() is available])
...
dnl #
		AC_MSG_CHECKING([whether drm_mode_object->free_cb is available])
		AC_KERNEL_TRY_COMPILE([
			#include <drm/drm_crtc.h>
		],[
			...
			AC_MSG_RESULT(yes)
			AC_DEFINE(HAVE_FREE_CB_IN_STRUCT_DRM_MODE_OBJECT, 1, [drm_mode_object->free_cb is available])
		],[
			AC_MSG_RESULT(no)
		])
	])
