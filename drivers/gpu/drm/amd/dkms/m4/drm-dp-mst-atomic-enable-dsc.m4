...
AC_DEFUN([AC_AMDGPU_DRM_DP_MST_ATOMIC_ENABLE_DSC],
	[AC_MSG_CHECKING([whether drm_dp_mst_atomic_enable_dsc() is available])
	AC_KERNEL_TRY_COMPILE_SYMBOL([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_DP_MST_ATOMIC_ENABLE_DSC, 1, [drm_dp_mst_atomic_enable_dsc() is available])
	], [
		AC_MSG_RESULT(no)
	])
])
