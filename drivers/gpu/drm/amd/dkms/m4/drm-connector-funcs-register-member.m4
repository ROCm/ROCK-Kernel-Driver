...
AC_DEFUN([AC_AMDGPU_DRM_CONNECTOR_FUNCS_REGISTER_MEMBER],
	[AC_MSG_CHECKING([whether struct drm_connector_funcs has register members])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_CONNECTOR_FUNCS_REGISTER, 1, [struct drm_connector_funcs has register members])
	], [
		AC_MSG_RESULT(no)
	])
])
