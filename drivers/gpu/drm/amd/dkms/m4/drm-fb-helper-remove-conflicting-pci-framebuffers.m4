...
AC_DEFUN([AC_AMDGPU_DRM_FB_HELPER_REMOVE_CONFLICTING_PCI_FRAMEBUFFERS],
	[AC_MSG_CHECKING([whether drm_fb_helper_remove_conflicting_pci_framebuffers() wants p,p args])
	AC_KERNEL_TEST_HEADER_FILE_EXIST([drm/drmP.h], [
		AC_KERNEL_TRY_COMPILE([
			...
			AC_MSG_RESULT(yes)
			AC_DEFINE(HAVE_DRM_FB_HELPER_REMOVE_CONFLICTING_PCI_FRAMEBUFFERS_PP, 1, [drm_fb_helper_remove_conflicting_pci_framebuffers() wants p,p args])
			...
			dnl #
			AC_MSG_CHECKING([whether drm_fb_helper_remove_conflicting_pci_framebuffers() wants p,i,p args])
			AC_KERNEL_TRY_COMPILE([
				#include <drm/drmP.h>
				#include <drm/drm_fb_helper.h>
			],[
				...
				AC_MSG_RESULT(yes)
				AC_DEFINE(HAVE_DRM_FB_HELPER_REMOVE_CONFLICTING_PCI_FRAMEBUFFERS_PIP, 1, [drm_fb_helper_remove_conflicting_pci_framebuffers() wants p,i,p args])
				AC_DEFINE(HAVE_DRM_FB_HELPER_REMOVE_CONFLICTING_PCI_FRAMEBUFFERS, 1, [drm_fb_helper_remove_conflicting_pci_framebuffers() is available])
			],[
				AC_MSG_RESULT(no)
				dnl #
				...
				AC_KERNEL_TRY_COMPILE_SYMBOL([
					...
					AC_MSG_RESULT(yes)
					AC_DEFINE(HAVE_REMOVE_CONFLICTING_FRAMEBUFFERS_RETURNS_INT, 1, [remove_conflicting_framebuffers() returns int])
				],[
					AC_MSG_RESULT(no)
				])
			])
