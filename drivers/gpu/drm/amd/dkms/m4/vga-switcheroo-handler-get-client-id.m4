...
AC_DEFUN([AC_AMDGPU_VGA_SWITCHEROO_GET_CLIENT_ID], [
	AC_MSG_CHECKING([whether vga_switcheroo_handler->get_client_id() return int])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_VGA_SWITCHEROO_GET_CLIENT_ID_RETURN_INT, 1, [vga_switcheroo_handler->get_client_id() return int])
	], [
		AC_MSG_RESULT(no)
	])
])
