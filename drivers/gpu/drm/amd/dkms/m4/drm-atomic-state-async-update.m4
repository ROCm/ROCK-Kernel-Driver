...
AC_DEFUN([AC_AMDGPU_DRM_ATOMIC_STATE_ASYNC_UPDATE], [
	AC_MSG_CHECKING([whether struct drm_atomic_state have async_update])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_ATOMIC_STATE_ASYNC_UPDATE, 1, [whether struct drm_atomic_state have async_update])
	], [
		AC_MSG_RESULT(no)
	])
])
