...
AC_DEFUN([AC_AMDGPU_DRM_ATOMIC_HELPER_DISABLE_ALL],
	[AC_MSG_CHECKING([whether drm_atomic_helper_disable_all() is available])
	AC_KERNEL_TRY_COMPILE_SYMBOL([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_ATOMIC_HELPER_DISABLE_ALL, 1, [drm_atomic_helper_disable_all() is available])
	], [
		AC_MSG_RESULT(no)
	])
])
