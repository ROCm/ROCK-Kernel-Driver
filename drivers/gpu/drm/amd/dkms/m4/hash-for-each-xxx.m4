...
AC_DEFUN([AC_AMDGPU_HASH_FOR_EACH_XXX],
	[AC_MSG_CHECKING([whether hash_for_each_xxx() drop the node parameter])
	AC_KERNEL_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_HASH_FOR_EACH_XXX_DROP_NODE, 1, [hash_for_each_xxx() drop the node parameter])
	],[
		AC_MSG_RESULT(no)
	])
])
