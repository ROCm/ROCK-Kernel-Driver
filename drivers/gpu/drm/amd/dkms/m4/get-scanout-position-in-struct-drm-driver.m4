...
AC_DEFUN([AC_AMDGPU_GET_SCANOUT_POSITION_IN_DRM_DRIVER],
	...
	AC_KERNEL_TEST_HEADER_FILE_EXIST([drm/drmP.h], [
		AC_KERNEL_TRY_COMPILE([
			...
			AC_DEFINE(HAVE_VGA_USE_UNSIGNED_INT_PIPE, 1, [get_scanout_position use unsigned int pipe])
		], [
			AC_MSG_RESULT(no)
			AC_MSG_CHECKING([whether get_scanout_position use unsigned int pipe])
			dnl #
