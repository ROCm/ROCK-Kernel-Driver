...
AC_DEFUN([AC_AMDGPU_DRM_CALC_VBLTIMESTAMP_FROM_SCANOUTPOS],
	[AC_MSG_CHECKING([whether drm_calc_vbltimestamp_from_scanoutpos() use ktimer_t arg])
	AC_KERNEL_TRY_COMPILE_SYMBOL([
		#include <drm/drmP.h>
	], [
		drm_calc_vbltimestamp_from_scanoutpos(NULL, 0, NULL, (ktimer_t *)NULL, 0);
	], [drm_calc_vbltimestamp_from_scanoutpos], [drivers/gpu/drm/drm_vblank.c], [
	], [
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_DRM_CALC_VBLTIMESTAMP_FROM_SCANOUTPOS_USE_KTIMER_T_ARG, 1, [drm_calc_vbltimestamp_from_scanoutpos() use ktimer_t arg])
	], [
		AC_MSG_RESULT(no)
...
dnl #
		AC_KERNEL_TRY_COMPILE_SYMBOL([
			...
			AC_MSG_RESULT(yes)
			AC_DEFINE(HAVE_DRM_CALC_VBLTIMESTAMP_FROM_SCANOUTPOS_DROP_MOD_ARG, 1, [drm_calc_vbltimestamp_from_scanoutpos() drop mode arg])
		], [
			AC_MSG_RESULT(no)
...
dnl #
			AC_KERNEL_TRY_COMPILE_SYMBOL([
				...
				AC_MSG_RESULT(yes)
				AC_DEFINE(HAVE_DRM_CALC_VBLTIMESTAMP_FROM_SCANOUTPOS_HAVE_MODE_ARG, 1, [drm_calc_vbltimestamp_from_scanoutpos() remove crtc arg])
			], [
				AC_MSG_RESULT(no)
...
dnl #
				AC_KERNEL_TRY_COMPILE_SYMBOL([
					...
					AC_MSG_RESULT(yes)
					AC_DEFINE(HAVE_DRM_CALC_VBLTIMESTAMP_FROM_SCANOUTPOS_HAVE_CRTC_MODE_ARG, 1, [drm_calc_vbltimestamp_from_scanoutpos() have the crtc & mode arg])
				], [
					AC_MSG_RESULT(no)
				])
			])
