/* $Id: os_4bri.c,v 1.1.2.2 2002/10/02 14:38:37 armin Exp $ */
  ...
  */
  ...
  for (i = 0; i < tasks; i++) {
    ...
    if (!diva_current->xdi_adapter.e_tbl) {
      ...
      diva_4bri_cleanup_adapter (a);
      for (i = 1; i < (tasks-1); i++) { diva_os_free (0, adapter_list[i]); }
      return (-1);
    }
...
void *
xdiLoadFile (char *FileName, unsigned long *FileLength, unsigned long lim)
{
  ...
  */
  ...
  if (!started) {
    DBG_FTL(("%s: Adapter selftest failed, signature=%04x",
              IoAdapter->Properties.Name, READ_WORD(&signature[0])))
