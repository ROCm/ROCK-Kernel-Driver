...
/*
  ...
  */
#if !defined(PCI_DEVICE_ID_EICON_4BRI)
#define PCI_DEVICE_ID_EICON_4BRI             0xE012
#endif

#if !defined(PCI_DEVICE_ID_EICON_4BRI_2)
#define PCI_DEVICE_ID_EICON_4BRI_2           0xE013
#endif
...
/* Diva Server 4BRI-8M PCI 0xE012 */
  { PCI_VENDOR_ID_EICON, PCI_DEVICE_ID_EICON_4BRI,
    PCI_ANY_ID, PCI_ANY_ID, 0, 0, CARDTYPE_DIVASRV_Q_8M_PCI },
/* Diva Server 4BRI-8M 2.0 PCI 0xE013 */
  { PCI_VENDOR_ID_EICON, PCI_DEVICE_ID_EICON_4BRI_2,
...
/* Diva Server Voice PRI 2.0 PCI 0xE019 */
  { PCI_VENDOR_ID_EICON, PCI_DEVICE_ID_EICON_MAESTRAP_2_VOIP,
    PCI_ANY_ID, PCI_ANY_ID, 0, 0, CARDTYPE_DIVASRV_VOICE_P_30M_V2_PCI },
...
void diva_init_dma_map (void* hdev,
	...
	struct _diva_dma_map_entry* pmap = diva_alloc_dma_map (hdev, nentries);
  ...
  if (pmap) {
    ...
    for (i = 0; i < nentries; i++) {
      ...
      diva_init_dma_map_entry (pmap, i, cpu_addr,
                               (dword)dma_handle, addr_handle);
      DBG_TRC(("dma map alloc [%d]=(%08lx:%08x:%08lx)",
               i, (unsigned long)cpu_addr, (dword)dma_handle, (unsigned long)addr_handle))
    }
  }
...
void diva_free_dma_map (void* hdev,
  ...
  for (i = 0; (pmap != 0); i++) {
    ...
    pci_free_consistent (pdev, PAGE_SIZE, addr_handle, dma_handle);
    DBG_TRC(("dma map free [%d]=(%08lx:%08x:%08lx)",
             i, (unsigned long)cpu_addr, (dword)dma_handle, (unsigned long)addr_handle))
  }
...
int diva_os_schedule_soft_isr (diva_os_soft_isr_t* psoft_isr)
{
  if (psoft_isr && psoft_isr->object) {
  	diva_os_thread_dpc_t* pdpc = (diva_os_thread_dpc_t*)psoft_isr->object;
...
static int DIVA_INIT_FUNCTION divas_register_chrdev(void)
{
		...
		devfs_handle = devfs_register(NULL, "Divas", DEVFS_FL_DEFAULT, major, 0,
...
static int __devinit divas_init_one (struct pci_dev *pdev,
  ...
  if (pci_enable_device (pdev)) {
    DBG_TRC(("%s: %s bus: %08x fn: %08x device init failed.\n",
           DRIVERLNAME, CardProperties[ent->driver_data].Name,
           pdev->bus->number, pdev->devfn))
    printk(KERN_ERR "%s: %s bus: %08x fn: %08x device init failed.\n",
           DRIVERLNAME, CardProperties[ent->driver_data].Name,
           pdev->bus->number, pdev->devfn);
    return (-EIO);
  }
  ...
  if (!pci_latency) {
    ...
    printk(KERN_INFO "%s: bus: %08x fn: %08x fix latency.\n",
           DRIVERLNAME, pdev->bus->number, pdev->devfn);
  ...
  if (!(pdiva = diva_driver_add_card (pdev, ent->driver_data))) {
    DBG_TRC(("%s: %s bus: %08x fn: %08x card init failed.\n",
           DRIVERLNAME, CardProperties[ent->driver_data].Name,
           pdev->bus->number, pdev->devfn))
    printk(KERN_ERR "%s: %s bus: %08x fn: %08x card init failed.\n",
           DRIVERLNAME, CardProperties[ent->driver_data].Name,
           pdev->bus->number, pdev->devfn);
    return (-EIO);
  }
