/* $Id: divamnt.c,v 1.1.2.2 2002/10/02 14:38:37 armin Exp $
 *
...
maint_read(struct file *file, char *buf, size_t count, loff_t * off)
{
  ...
  if (!file->private_data) {
    for (;;) {
      while ((pmsg = diva_maint_get_message (&size, &old_irql))) {
    ...
    /*
      ...
      */
    if (!(str_msg = (int*)diva_os_malloc_tbuffer(0, pmsg->data_length + 29 + 2*sizeof(int)))) {
...
static int DIVA_INIT_FUNCTION divas_maint_register_chrdev(void)
{
		...
		devfs_handle = devfs_register(NULL, "DivasMAINT", DEVFS_FL_DEFAULT, major, 0,
					S_IFCHR | S_IRUSR | S_IWUSR, &divas_maint_fops, NULL);
...
static int DIVA_INIT_FUNCTION maint_init(void)
{
  ...
  printk("%s  Build: %s  Major: %d\n", getrev(tmprev), DIVA_BUILD, major);
  ...
  printk(KERN_INFO "%s: trace buffer = %p - %d kBytes, %s \n",
         DRIVERLNAME, buffer, buffer_length, (diva_dbg_mem == 0) ? "internal" : "external");
