...
static void diva_register_appl(struct capi_ctr *, __u16 , capi_register_params *);
...
void sendf(APPL *appl, word command, dword Id, word Number, byte *format, ...)
{
  ...
  if (command == _DATA_B3_I)
	...
	if (!(dmb = diva_os_alloc_message_buffer(length + dlength, (void **)&write))) {
  ...
  if(!(card = find_card_by_ctrl(write[8] & 0x7f))) {
    DBG_ERR(("sendf - controller %d not found, incoming msg dropped", write[8] & 0x7f))
    ...
    if (a->li_base < k)
    {
      ...
      for (i = 0; i < k; i++)
      {
        memmove(&li_config_table[i].flag_table[a->li_base],
                ...
                k - a->li_base);
        memmove(&li_config_table[i].coef_table[a->li_base],
...
static void divacapi_remove_card(DESCRIPTOR *d)
{
  ...
  while(card) {
    if (card->d.request == d->request)
    {
			detach_capi_ctr(&card->capi_ctrl);
      clean_adapter(card->Id - 1);
      DBG_TRC(("DelAdapterMap (%d) -> (%d)", ControllerMap[card->Id], card->Id))
...
static int diva_add_card(DESCRIPTOR *d)
{
  ...
  if (!(a->plci = (PLCI *) diva_os_malloc(0, sizeof(PLCI) * a->max_plci))) {
  ...
  k = li_total_channels + a->li_channels;
  new_li_config_table = (LI_CONFIG *) diva_os_malloc(0,
    ((k * sizeof(LI_CONFIG) + 3) & ~3) + (2 * k) * ((k + 3) & ~3));
  ...
  for (i = 0; i < k; i++)
  {
    ...
    new_li_config_table[i].flag_table = ((byte *) new_li_config_table) +
      (((k * sizeof(LI_CONFIG) + 3) & ~3) + (2 * i) * ((k + 3) & ~3));
    new_li_config_table[i].coef_table = ((byte *) new_li_config_table) +
      (((k * sizeof(LI_CONFIG) + 3) & ~3) + (2 * i + 1) * ((k + 3) & ~3));
    if ((i >= a->li_base) && (i < a->li_base + a->li_channels))
    {
    ...
    else
    {
      ...
      {
        memcpy(&new_li_config_table[i].flag_table[a->li_base + a->li_channels],
               &li_config_table[j].flag_table[a->li_base],
               k - (a->li_base + a->li_channels));
        memcpy(&new_li_config_table[i].coef_table[a->li_base + a->li_channels],
 ...
 *  register appl
 */
static void diva_register_appl(struct capi_ctr *ctrl, __u16 appl, capi_register_params *rp)
{
  APPL *this;
  word bnum, xnum;
  int i = 0, j = 0;
...
static void diva_release_appl(struct capi_ctr *ctrl, __u16 appl)
{
  ...
  {
    CapiRelease(this->Id);
    if (this->DataNCCI) diva_os_free(0, this->DataNCCI);
    if (this->DataFlags) diva_os_free(0, this->DataFlags);
    if (this->ReceiveBuffer) diva_os_free(0, this->ReceiveBuffer);
    if (this->xbuffer_ptr)
    {
      for (i = 0; i < (MAX_DATA_B3 * this->MaxNCCI); i++)
      {
        if (this->xbuffer_ptr[i]) diva_os_free(0, this->xbuffer_ptr[i]);
      }
      diva_os_free(0, this->xbuffer_ptr);
    }
    if (this->xbuffer_internal) diva_os_free(0, this->xbuffer_internal);
    if (this->xbuffer_used) diva_os_free(0, this->xbuffer_used);
    this->Id = 0;
  }
...
static u16 diva_send_message(struct capi_ctr *ctrl, diva_os_message_buffer_s *dmb)
{
  ...
  switch ( command )
  {
    ...
    case _DATA_B3_R:
      ...
      if (clength == 24) clength = 22; /* workaround for PPcom bug */
                                      /* header is always 22      */
      if (READ_WORD(&msg->info.data_b3_req.Data_Length) > this->MaxDataLength ||
      ...
      {
        for ( j = 0 ; j < READ_WORD(&msg->info.data_b3_req.Data_Length) ; j += 256 )
        {
          DBG_BLK((((char *)this->xbuffer_ptr[i]) + j,
            ((READ_WORD(&msg->info.data_b3_req.Data_Length) - j) < 256) ?
             (READ_WORD(&msg->info.data_b3_req.Data_Length) - j) : 256  ))
          if ( !(myDriverDebugHandle.dbgMask & DL_PRV0) )
            break ; // not more if not explicitely requested
        }
