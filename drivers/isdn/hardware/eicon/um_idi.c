/* $Id: um_idi.c,v 1.1.2.2 2002/10/02 14:38:38 armin Exp $ */
...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
static void cleanup_adapter (diva_um_idi_adapter_t* a) {
  divas_um_idi_entity_t* e = (divas_um_idi_entity_t*)diva_q_get_head	(&a->entity_q);
  ...
  if (e->status & DIVA_UM_IDI_RC_PENDING) {
	...
	switch (req->type) {
		case DIVA_UM_IDI_GET_FEATURES: {
			...
			if (!(data = diva_data_q_get_segment4write	(&e->data))) {
			...
			diva_data_q_ack_segment4write	(&e->data, sizeof(diva_um_idi_ind_hdr_t));
		...
		if (Req != ASSIGN) {
			...
			switch (type) {
				case DIVA_UM_IDI_REQ_TYPE_MAN:
					e->e.Id = MAN_ID;
					DBG_TRC(("A(%d) E(%08x) assign MAN", e->adapter->adapter_nr, e));
					break;
...
static int process_idi_rc (divas_um_idi_entity_t* e, byte rc) {
	...
	if (e->status & DIVA_UM_IDI_ASSIGN_PENDING) {
		e->status &= ~DIVA_UM_IDI_ASSIGN_PENDING;
		if (rc != ASSIGN_OK) {
			DBG_ERR(("A: A(%d) E(%08x) ASSIGN failed", e->adapter->adapter_nr, e));
			e->e.callback = 0;
			e->e.Id = 0;
			e->e.Req = 0;
			e->e.ReqCh = 0;
...
static int process_idi_ind (divas_um_idi_entity_t* e, byte ind) {
	...
	if (e->e.complete != 0x02) {
		diva_um_idi_ind_hdr_t* pind =
					(diva_um_idi_ind_hdr_t*)diva_data_q_get_segment4write (&e->data);
		if (pind) {
			e->e.RNum       = 1;
			e->e.R->P			  = (byte*)&pind[1];
			e->e.R->PLength = (word)(diva_data_q_get_max_length	(&e->data) -\
												sizeof(*pind));
			DBG_TRC(("A(%d) E(%08x) ind_1(%02x-%02x-%02x)-[%d-%d]", \
							e->adapter->adapter_nr, e, \
							e->e.Id, ind, e->e.IndCh, e->e.RLength, e->e.R->PLength));

		} else {
			DBG_TRC(("A(%d) E(%08x) ind(%02x-%02x-%02x)-RNR", \
								e->adapter->adapter_nr, e, \
								e->e.Id, ind, e->e.IndCh));
			e->e.RNum	= 0;
			e->e.RNR	= 1;
			do_wakeup = 1;
		}
		...
		DBG_TRC(("A(%d) E(%08x) ind(%02x-%02x-%02x)-[%d]", \
							e->adapter->adapter_nr, e, \
							e->e.Id, ind, e->e.IndCh, e->e.R->PLength));
...
static int	write_return_code (divas_um_idi_entity_t* e, byte rc) {
	...
	if (!(prc = (diva_um_idi_ind_hdr_t*)diva_data_q_get_segment4write	(&e->rc))) {
		DBG_ERR (("A: A(%d) E(%08x) rc(%02x) lost", e->adapter->adapter_nr, e, rc));
		e->status &= ~DIVA_UM_IDI_RC_PENDING;
		return (-1);
	}
