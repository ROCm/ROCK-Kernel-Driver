...
static DESCRIPTOR MaintDescriptor = { IDI_DIMAINT, 0, 0, (IDI_CALL)diva_maint_prtComp };

extern void *diva_os_malloc_tbuffer(unsigned long flags, unsigned long size);
extern void diva_os_free_tbuffer(unsigned long flags, void* ptr);
extern int diva_os_copy_to_user(void* os_handle, void* dst, const void* src, int length);
extern int diva_os_copy_from_user(void* os_handle, void* dst, const void* src, int length);
...
int maint_read_write(void * buf)
{
  ...
  if (diva_os_copy_from_user(NULL, (void *)&data[0], buf, 3*sizeof(dword))) {
    return(-EFAULT);
  }
  ...
  switch (cmd) {
    case DITRACE_CMD_GET_DRIVER_INFO:
      if ((ret = diva_get_driver_info (id, data, sizeof(data))) > 0) {
        if (diva_os_copy_to_user(NULL, buf, (void*)&data[0], ret))
          ret = -EFAULT;
      } else {
        ret = -EINVAL;
      }
      ...
      if ((pmsg = diva_maint_get_message (&size, &old_irql))) {
        if (size > mask) {
          ...
          if (!(pbuf = diva_os_malloc_tbuffer(0, size))) {
            ...
            if (diva_os_copy_to_user(NULL, buf, (void*)pbuf, size))
              ret = -EFAULT;
 ...
 *  init
 */
int DIVA_INIT_FUNCTION mntfunc_init(int *buffer_length, void **buffer, unsigned long diva_dbg_mem)
{
  if (*buffer_length < 64) {
    *buffer_length = 64;
  }
  ...
  *buffer_length *= 1024;
  ...
  if (diva_dbg_mem) {
    *buffer = (void*)diva_dbg_mem;
    ...
    while ((*buffer_length >= (64*1024)) && (!(*buffer = diva_os_malloc_tbuffer(0, *buffer_length)))) {
      *buffer_length -= 1024;
    }
