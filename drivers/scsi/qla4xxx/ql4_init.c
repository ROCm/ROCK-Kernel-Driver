...
void ql4xxx_set_mac_number(scsi_qla_host_t * ha)
{
	...
	switch (value & ISP_CONTROL_FN_MASK) {
        ...
        case ISP_CONTROL_FN1_SCSI:
            ...
            break;
        default:
            DEBUG2(printk(KERN_INFO"scsi%d: %s: Invalid function number, ispControlStatus = 0x%x\n",
			  ha->host_no, __func__,value));
            break;
    }
	DEBUG2(printk(KERN_INFO"scsi%d: %s: mac_index %d.\n",  ha->host_no,__func__,ha->mac_index)) ;
}
		 ...
		 */
 		...
 		if (ddb_state == DDB_DS_SESSION_FAILED ||
			...
			if (err_code == 0x1c || err_code == 0x06) {
				DEBUG2(printk("%s send target completed"
...
qla4xxx_initialize_adapter(scsi_qla_host_t *ha, uint8_t renew_ddb_list)
{
	...
	if (!ha->tot_ddbs) {
			QL4PRINT(QLP2, printk(KERN_WARNING "scsi%d:"
