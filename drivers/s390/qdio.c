...
#define VERSION_QDIO_C "$Revision: 1.122 $"
...
static unsigned long i_p_c=0,i_p_nc=0,o_p_c=0,o_p_nc=0,ii_p_c=0,ii_p_nc=0;
...
/* iQDIO stuff: */
static volatile qdio_q_t *iq_list=NULL; /* volatile as it could change
					   during a while loop */
...
#define my_spin_lock(x) do { \
	void *ptr=x; \
	QDIO_DBF_TEXT6(0,trace,"sp_lck"); \
	QDIO_DBF_HEX6(0,trace,&ptr,sizeof(void*)); \
	spin_lock(x); \
} while (0)
#define my_spin_unlock(x) do { \
	void *ptr=x; \
	QDIO_DBF_TEXT6(0,trace,"sp_unlck"); \
	QDIO_DBF_HEX6(0,trace,&ptr,sizeof(void*)); \
	spin_unlock(x); \
} while (0)
#define my_spin_lock_irqsave(x,y) do { \
	void *ptr=x; \
	QDIO_DBF_TEXT6(0,trace,"sp_lck_i"); \
	QDIO_DBF_HEX6(0,trace,&ptr,sizeof(void*)); \
	spin_lock_irqsave(x,y); \
} while (0)
#define my_spin_unlock_irqrestore(x,y) do { \
	void *ptr=x; \
        QDIO_DBF_TEXT6(0,trace,"sp_nlk_i"); \
	QDIO_DBF_HEX6(0,trace,&ptr,sizeof(void*)); \
	spin_unlock_irqrestore(x,y); \
} while (0)
#define my_s390irq_spin_lock_irqsave(x,y) do { \
	void *ptr=x; \
	QDIO_DBF_TEXT6(0,trace,"sp_lck3i"); \
	QDIO_DBF_HEX6(0,trace,&ptr,sizeof(void*)); \
	s390irq_spin_lock_irqsave(x,y); \
} while (0)
#define my_s390irq_spin_unlock_irqrestore(x,y) do { \
	void *ptr=x; \
        QDIO_DBF_TEXT6(0,trace,"sp_nlk3i"); \
	QDIO_DBF_HEX6(0,trace,&ptr,sizeof(void*)); \
	s390irq_spin_unlock_irqrestore(x,y); \
} while (0)
...
#define my_s390irq_spin_lock_irqsave(x,y) s390irq_spin_lock_irqsave(x,y)
#define my_s390irq_spin_unlock_irqrestore(x,y) s390irq_spin_unlock_irqrestore(x,y)
...
static inline volatile void iqdio_clear_summary_bit(__u32 *location)
{
#ifdef QDIO_DBF_LIKE_HELL
...
static inline volatile void iqdio_set_summary_bit(__u32 *location)
{
#ifdef QDIO_DBF_LIKE_HELL
...
static inline void iqdio_sched_tl(void)
{
	tasklet_hi_schedule(&iqdio_tasklet);
}

static inline void qdio_mark_iq(qdio_q_t *q)
{
	unsigned long flags;
#ifdef QDIO_DBF_LIKE_HELL
	...
	my_spin_lock_irqsave(&iq_list_lock,flags);
	if (atomic_read(&q->is_in_shutdown)) goto out_unlock;
	...
	if (q->is_input_q) {
		...
		my_spin_unlock_irqrestore(&iq_list_lock,flags);
...
static inline void qdio_mark_q(qdio_q_t *q)
{
	...
	if (atomic_read(&q->is_in_shutdown)) return;
...
static inline volatile unsigned long iqdio_clear_global_summary(void)
{
	unsigned long time;
...
static inline void iqdio_inbound_processing(qdio_q_t *q)
{
#ifdef QDIO_DBF_LIKE_HELL
...
static inline void iqdio_inbound_checks(void)
{
	qdio_q_t *q;
#ifdef QDIO_USE_PROCESSING_STATE
	int q_laps=0;
...
static void iqdio_check_for_polling(void)
{
}

static void iqdio_tl(unsigned long data)
{
#ifdef QDIO_DBF_LIKE_HELL
...
static int iqdio_thinint_handler(__u32 intparm)
{
#ifdef QDIO_DBF_LIKE_HELL
...
/* FIXME: this is never used in the kernel, why? */
...
static unsigned int iqdio_check_chsc_availability(void) {
	int result;
	int i;
...
/* the chsc_area is locked by the lock in qdio_activate */
static unsigned int iqdio_set_subchannel_ind(qdio_irq_t *irq_ptr,
					     int reset_to_zero)
{
...
static unsigned int iqdio_set_delay_target(qdio_irq_t *irq_ptr,
...
int qdio_cleanup(int irq,int how)
{
	...
	my_spin_lock(&irq_ptr->setting_up_lock);
	...
	/* cleanup subchannel */
	my_s390irq_spin_lock_irqsave(irq,flags);
	...
	my_s390irq_spin_unlock_irqrestore(irq,flags);
	...
	if (irq_ptr->is_iqdio_irq) {
		...
		iqdio_set_subchannel_ind(irq_ptr,1); /* reset adapter
...
/*		irq_ptr->original_int_handler=NULL; */
	...
	my_spin_unlock(&irq_ptr->setting_up_lock);
...
static unsigned long qdio_get_characteristics(int irq)
{
	qdio_irq_t *irq_ptr;
	int result=0;
...
#define DEVSTAT_FLAG_IRQ_QDIO ~0UL
	...
	for (i=0;i<init_data->no_input_qs;i++) {
		...
		irq_ptr->qdr->qdf0[i].sliba=
			(unsigned long)(irq_ptr->input_qs[i]->slib);

		irq_ptr->qdr->qdf0[i].sla=
			(unsigned long)(irq_ptr->input_qs[i]->sl);

		irq_ptr->qdr->qdf0[i].slsba=
			(unsigned long)(&irq_ptr->input_qs[i]->
					slsb.acc.val[0]);
	...
	for (i=0;i<init_data->no_output_qs;i++) {
		...
		irq_ptr->qdr->qdf0[i+init_data->no_input_qs].sla=
			(unsigned long)(irq_ptr->output_qs[i]->sl);

		irq_ptr->qdr->qdf0[i+init_data->no_input_qs].slsba=
			(unsigned long)(&irq_ptr->output_qs[i]->
					slsb.acc.val[0]);
	...
	my_s390irq_spin_lock_irqsave(irq_ptr->irq,saveflags);
...
/*		iqdio_enable_adapter_int_facility(irq_ptr);*/
	...
	result=do_IO(irq_ptr->irq,&irq_ptr->ccw,QDIO_DOING_ESTABLISH,0,0);
	if (result) {
				   ...
				   void *data)
{
	...
	/* FIXME: wtf are you trying to do? this is guaranteed to
	 * fail, you even get a compiler warning! */
	irq_ptr=first_irq;
...
static void iqdio_register_thinints(void)
{
	char dbf_text[20];
...
#ifdef MODULE
static void iqdio_unregister_thinints(void)
{
	if (!register_thinint_result)
