				  ...
				  uid_t uid, gid_t gid)
{
	...
	if ((uid != 0) || (gid != 0))
		kmsg_warn("Root becomes the owner of all s390dbf files "
			  "in sysfs\n");
	if (!initialized)
		BUG();
	mutex_lock(&debug_mutex);
...
out:
        if (!rc){
		kmsg_err("Registering debug feature %s failed\n", name);
        }
	mutex_unlock(&debug_mutex);
	return rc;
}
...
debug_set_size(debug_info_t* id, int nr_areas, int pages_per_area)
{
	...
	if(pages_per_area > 0){
		...
		if(!new_areas) {
			kmsg_info("Allocating memory for %i pages failed\n",
				  pages_per_area);
			rc = -ENOMEM;
			goto out;
		}
	...
	kmsg_info("%s: set new size (%i pages)\n" ,id->name, pages_per_area);
out:
	return rc;
}
...
debug_set_level(debug_info_t* id, int new_level)
{
	...
	spin_lock_irqsave(&id->lock,flags);
        if(new_level == DEBUG_OFF_LEVEL){
                ...
                kmsg_info("%s: switched off\n",id->name);
        } else if ((new_level > DEBUG_MAX_LEVEL) || (new_level < 0)) {
                kmsg_info("%s: level %i is out of range (%i - %i)\n",
...
debug_register_view(debug_info_t * id, struct debug_view *view)
{
	...
	pde = debugfs_create_file(view->name, mode, id->debugfs_root_entry,
				id , &debug_file_ops);
	if (!pde){
		kmsg_err("Registering view %s/%s failed due to out of "
			 "memory\n", id->name,view->name);
		rc = -1;
		goto out;
	}
	...
	if (i == DEBUG_MAX_VIEWS) {
		kmsg_err("Registering view %s/%s would exceed the maximum "
			 "number of views %i\n", id->name, view->name, i);
		debugfs_remove(pde);
		rc = -1;
	} else {
			...
			size_t user_len, loff_t * offset)
{
	...
	if(new_level < 0) {
		kmsg_warn("%s is not a valid level for a debug "
			  "feature\n", str);
		rc = -EINVAL;
	} else {
			...
			size_t user_len, loff_t * offset)
{
	...
	if (user_len > 0x10000)
        ...
        kmsg_info("Flushing debug data failed because %c is not a valid "
		  "area\n", input_buf[0]);
