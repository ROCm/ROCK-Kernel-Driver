 ...
 * kdba_prologue
 *
 *	FIXME:	On ia64 this function is not used yet.  It should be used
 *		so every arch has a common interface.  For the moment,
 *		kdba_bt uses ia64 specific unwind code.
 *
 ...
 *	None.
 * Remarks:
 *	FIXME: design an activation record format that works for all architectures.
 */
...
KDBA_UNWIND_HANDLER(do_kdba_main_loop, struct kdba_main_loop_data, data->reason,
...
int
kdb_tpa(int argc, const char **argv, const char **envp, struct pt_regs* regs)
{
	kdb_machreg_t addr;
        int diag;
	long offset = 0;
...
int
kdb_tpav(int argc, const char **argv, const char **envp, struct pt_regs* regs)
{
...
kdba_init(void)
{
	...
	kdb_register("tpa", kdb_tpa, "<vaddr>", "Translate virtual to physical address", 0);
#if defined(CONFIG_NUMA)
	kdb_register("tpav", kdb_tpav, "<begin addr> <end addr>", "Verify that physical addresses corresponding to virtual addresses from <begin addr> to <end addr> are in same node", 0);
#endif
