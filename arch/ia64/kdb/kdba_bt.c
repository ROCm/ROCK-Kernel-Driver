	     ...
	     struct unw_frame_info *info /* FIXME: should be part of ar for ia64 */)
{
	...
	if (btsp)
		kdb_printf("        sp 0x%016lx bsp 0x%016lx cfm 0x%016lx\n", sp, bsp, cfm);
}
...
kdba_bt_stack(int argcount, struct task_struct *p)
{
	...
	u64 *saved_pfs_loc = NULL, use_saved_pfs_loc = 0, zero = 0;
	...
	do {
		...
		/* WAR for spinlock contention from leaf functions */
		if (use_saved_pfs_loc)
			info.pfs_loc = saved_pfs_loc;
		use_saved_pfs_loc = 0;
		if (info.ip >= (u64)ia64_spinlock_contention_pre3_4 && info.ip < (u64)ia64_spinlock_contention_pre3_4_end) {
			saved_pfs_loc = info.pfs_loc;
			use_saved_pfs_loc = 1;
			info.pfs_loc = &zero;
		}
