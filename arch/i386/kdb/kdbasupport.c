	      ...
	      kdb_machreg_t fp, kdb_machreg_t ss, int caller, kdb_ar_t *ar)
{
	 ...
	 * instructions that were eventually followed by jmp restore_all,
	 * take the label before each such instruction.
	 */
	if (caller &&
	    (strcmp(symtab->sym_name, "ret_from_fork") == 0 ||
	     strcmp(symtab->sym_name, "need_resched") == 0 ||
	     strcmp(symtab->sym_name, "sysenter_past_esp") == 0 ||
	     strcmp(symtab->sym_name, "syscall_call") == 0 ||
	     strcmp(symtab->sym_name, "work_resched") == 0 ||
	     strcmp(symtab->sym_name, "work_notifysig") == 0 ||
	     strcmp(symtab->sym_name, "work_notifysig_v86") == 0 ||
	     strcmp(symtab->sym_name, "syscall_trace_entry") == 0 ||
	     strcmp(symtab->sym_name, "syscall_exit_work") == 0 ||
	     strcmp(symtab->sym_name, "common_interrupt") == 0 ||

	     /* from include/asm-i386/mach-default/entry_arch.h */
	     strcmp(symtab->sym_name, "reschedule_interrupt") == 0 ||
	     strcmp(symtab->sym_name, "invalidate_interrupt") == 0 ||
	     strcmp(symtab->sym_name, "call_function_interrupt") == 0 ||
	     strcmp(symtab->sym_name, "apic_timer_interrupt") == 0 ||
	     strcmp(symtab->sym_name, "error_interrupt") == 0 ||
	     strcmp(symtab->sym_name, "spurious_interrupt") == 0 ||
	     strcmp(symtab->sym_name, "thermal_interrupt") == 0 ||

	     strcmp(symtab->sym_name, "kdb_call") == 0 ||
	     strcmp(symtab->sym_name, "kdb_interrupt") == 0 ||
	     strcmp(symtab->sym_name, "error_code") == 0 ||
	     strcmp(symtab->sym_name, "device_not_available") == 0 ||
	     strcmp(symtab->sym_name, "device_not_available_emulate") == 0 ||
	     strcmp(symtab->sym_name, "debug_stack_correct") == 0 ||
	     strcmp(symtab->sym_name, "nmi_stack_correct") == 0 ||
	     strcmp(symtab->sym_name, "int3") == 0)) {
