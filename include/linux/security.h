...
#define COND_SECURITY_MAYBE(seop, def)		\
	(unlikely(security_enabled))? security_ops->seop: def
#define COND_SECURITY_OFF(seop, def)		\
	def

#ifdef CONFIG_SECURITY
# define COND_SECURITY(seop, def) COND_SECURITY_MAYBE(seop, def)
#else
# define COND_SECURITY(seop, def) COND_SECURITY_OFF(seop, def)
#endif
...
static inline void security_socket_post_create(struct socket * sock, 
					       ...
					       int protocol, int kern)
{
	COND_SECURITY(socket_post_create(sock, family, type,
					 protocol, kern),
		SE_NOP);
}
...
{
	return COND_SECURITY(sk_getsid(sk, fl, dir),
		0);
}
...
{
	return COND_SECURITY(xfrm_policy_alloc_security(xp, sec_ctx),
		0);
}

static inline int security_xfrm_policy_clone(struct xfrm_policy *old, struct xfrm_policy *new)
{
	return COND_SECURITY(xfrm_policy_clone_security(old, new),
		0);
}

static inline void security_xfrm_policy_free(struct xfrm_policy *xp)
{
	COND_SECURITY(xfrm_policy_free_security(xp),
		SE_NOP);
}

static inline int security_xfrm_state_alloc(struct xfrm_state *x, struct xfrm_user_sec_ctx *sec_ctx)
{
	return COND_SECURITY(xfrm_state_alloc_security(x, sec_ctx),
		0);
}

static inline void security_xfrm_state_free(struct xfrm_state *x)
{
	COND_SECURITY(xfrm_state_free_security(x),
		SE_NOP);
}

static inline int security_xfrm_policy_lookup(struct xfrm_policy *xp, u32 sk_sid, u8 dir)
{
	return COND_SECURITY(xfrm_policy_lookup(xp, sk_sid, dir),
		0);
}
...
{
	return COND_SECURITY(key_alloc(key),
		0);
}
