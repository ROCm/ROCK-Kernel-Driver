#ifndef __ASM_IA64_DPROBES_H__
#define __ASM_IA64_DPROBES_H__
...
struct dprobes_struct {
	...
	/*
	 *  psr save / restore area.
	 *
	 *  Both saved_psr and old_psr are initialized to capture
	 *  the state of the interrupt and single step bits in
	 *  the psr from the stack frame in the break instruction
	 *  fault handler.
	 *
	 *  old_psr is
	 *      - used to restore the psr when the single stepped
	 *        instruction faults.
	 *
	 *  saved_psr is
	 *      - used to restore the psr when the single stepped
	 * is executed successfully.
	 * old_eflags is used when the single-stepped instruction faults.
	 *        instruction is successfully executed.
	 *      - edited to mask out the interrupt bit if the
	 *        the instruction being single stepped can modify
	 *        the interrupt bit
	 */
...
extern int insert_probe_userspace(byte_t *addr, struct dp_record_struct *rec,
	struct page *page, struct vm_area_struct *vma);
extern int remove_probe_userspace(byte_t *addr, struct dp_record_struct *rec,
	struct page *page, struct vm_area_struct *vma);
extern int register_userspace_probes(struct dp_record_struct *rec);
extern void unregister_userspace_probes(struct dp_record_struct *rec);
